...
const UserAuthForm = ({ type }) => {
  ...
  const { setUserId, setIsAuth, setUser, userRole } = useUserContext();
  ...
  const userAuth = async (serverRoute, formData) => {
    if (serverRoute === "login") {
      try {
        ...
        response.data.user.role === roleOptions[0].value ? navigate("/patient-dashboard") : navigate("/doctor-dashboard");
